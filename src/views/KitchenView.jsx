...
export default function KitchenView({ onChangeMode }) {
  ...
  const handleTableClick = async (tableNumber) => {
    ...
    switch (table.status) {
      case 'ordered': await confirmCooking(tableNumber); break;
      case 'cooking': await markTableReady(tableNumber); break;
      case 'ready':   await resetTable(tableNumber); break;
      default: break;
    }
  };
                ...
                <section key={floor.floor}>
                  ...
                  <div className={`grid gap-3 ${
                    ...
                    {floor.tables.map((num) => (
                      <TableCard key={num} tableNumber={num} status={getTable(num).status || 'idle'} onClick={() => handleTableClick(num)} variant="kitchen" />
                    ))}
                  </div>
                </section>
              );
